package com.finger.support;

public class Constant {
    public static final String FILE_CITIES       = "file_city";
    public static final String FILE_TAGS         = "file_tag";
    public static final String FILE_ARTIST_GRADE = "file_grade";
    /**
     * 保存类型是
     * {@code HashMap<String,String>}
     * keySet:phone_num,password，type
     */
    public static final String FILE_ROLE_MAP     = "file_role_map";
    public static final String FILE_ROLE         = "file_role_instance";
    public static final String LOGIN_TYPE_ARTIST = "1";
    public static final String LOGIN_TYPE_USER   = "2";
    public static final String LOGIN_TYPE_EMPTY  = "0";

    public static final String COMMENT_GOOD = "1";
    public static final String COMMENT_MID  = "0";
    public static final String COMMENT_BAD  = "-1";


}
